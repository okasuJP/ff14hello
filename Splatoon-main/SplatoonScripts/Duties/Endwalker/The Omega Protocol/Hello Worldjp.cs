        ...
        public override void OnUpdate()
        {
            ...
            if (Svc.Objects.Any(x => x is BattleChara b && b.CastActionId == 31599))
            {
                ...
                if (HasEffect(Effects.RedRot))
                {
                    ...
                    {
                        if(Conf.EnableOverheadHintsGeneric) Reminder("サークル: 赤い塔の端", ImGuiColors.DalamudRed);
                    }
                    else
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("シェア： 赤い塔を踏む", ImGuiColors.DalamudRed);
                    }
                }
                else if (HasEffect(Effects.BlueRot))
                {
                    ...
                    if (HasEffect(Effects.Defamation))
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("サークル: 青い塔の端", ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if (Conf.EnableOverheadHintsGeneric) Reminder("シェア： 赤い塔を踏む", ImGuiColors.TankBlue);
                    }
                }
                else if (HasEffect(Effects.UpcomingCloseTether, 10f))
                {
                    ...
                    if (counter != 4)
                    {
                        ...
                        if (isDefamationRed)
                        {
                            ...
                            if (Conf.EnableOverheadHintsGeneric) Reminder("赤い塔から離れる → 受け取り", ImGuiColors.DalamudRed);
                        }
                        else
                        {
                            if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("青い塔から離れる → 受け取り", ImGuiColors.TankBlue);
                        }
                        if(partner != null)
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("青い塔の間 - final stack or avoid", ImGuiColors.TankBlue);
                        }
                        else
                        {
                            if (Conf.EnableVisualElementsTowers) TowerRed(true);
                            if (Conf.EnableOverheadHintsGeneric) Reminder("赤い塔の間 - final stack or avoid", ImGuiColors.DalamudRed);
                        }
                    }
                }
                else if (HasEffect(Effects.UpcomingFarTether, 10))
                {
                    ...
                    {
                        if (Conf.EnableVisualElementsTowers) TowerBlue(true);
                        if (Conf.EnableOverheadHintsGeneric) Reminder("青い塔の間" + (counter==4? " → 最後に重なる" : " → 初めに重なる"), ImGuiColors.TankBlue);
                    }
                    else
                    {
                        if (Conf.EnableVisualElementsTowers) TowerRed(true);
                        if (Conf.EnableOverheadHintsGeneric) Reminder("赤い塔の間" + (counter == 4 ? " → 最後に重なる" : " → 初めに重なる"), ImGuiColors.DalamudRed);
                    }
                }
            }
            else
            {
                ...
                if (RotPicker)
                {
                    if(Conf.EnableRotPickerReminding) Reminder("受け取る", 0xFF000000.ToVector4());
